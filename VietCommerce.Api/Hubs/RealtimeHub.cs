...
namespace VietCommerce.Api.Hubs
{
    [Authorize]
    public class RealtimeHub : Hub
    {
        ...
        {
            return Clients.Groups($"domain:orders", $"store:{order.StoreId}").SendAsync("OrderCreated", order);
        }

        public Task OrderStatusUpdated(Guid orderId, int status)
        {
            return Clients.Group("domain:orders").SendAsync("OrderStatusUpdated", new { orderId, status });
        }

        public Task InventoryUpdated(InventoryDto inventory)
        {
            return Clients.Groups("domain:inventory", $"store:{inventory.StoreId}").SendAsync("InventoryUpdated", inventory);
        }

        public Task TaskUpdated(TaskDto task)
        {
            return Clients.Groups("domain:tasks", $"user:{task.AssignedTo}").SendAsync("TaskUpdated", task);
        }
    }
}
