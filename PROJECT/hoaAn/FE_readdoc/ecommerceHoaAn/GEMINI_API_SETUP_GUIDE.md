...
6. Nếu thành công → API key đã hoạt động ✅
    ...
    "contents": [
      {
        "parts": [
          {
            "text": "Hello, Gemini! Hãy giới thiệu về bạn bằng tiếng Việt."
          }
        ]
      }
    ]
...
const result = await generateText({
  prompt: 'Viết một bài viết về AI',
  model: 'gemini-2.5-flash',
  temperature: 0.7,
  maxTokens: 8192,
});
...
### `generateMarketingContent(productInfo)`
...
// Returns: { title, content, hashtags }
```

### `generateSocialMediaPosts(baseContent, platforms)`
...
// Returns: { facebook: '...', instagram: '...', twitter: '...' }
```

### `improveMarketingContent(currentContent, improvements)`

Improve existing marketing content

```typescript
const result = await improveMarketingContent(
  'Nội dung hiện tại',
  ['Thêm emoji', 'Viết ngắn gọn hơn', 'Kêu gọi hành động']
);
```
...
### `generateSEOContent(productInfo)`
...
// Returns: { metaTitle, metaDescription, metaKeywords }
```
