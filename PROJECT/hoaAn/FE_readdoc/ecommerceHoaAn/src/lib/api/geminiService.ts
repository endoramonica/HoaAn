...
export async function generateMarketingContent(productInfo: {
    ...
    const prompt = `Bạn là một chuyên gia marketing. Hãy tạo nội dung quảng cáo cho sản phẩm sau:
...
export async function generateSocialMediaPosts(
    ...
    const prompt = `Dựa trên nội dung marketing sau:
"${baseContent}"
...
export async function improveMarketingContent(
    ...
    const prompt = `Nội dung marketing hiện tại:
"${currentContent}"
...
export async function generateProductDescription(productInfo: {
    ...
    const prompt = `Tạo mô tả sản phẩm hấp dẫn cho:
Tên: ${name}
${category ? `Danh mục: ${category}` : ''}
...
export async function generateSEOContent(productInfo: {
    ...
    const prompt = `Tạo nội dung SEO cho sản phẩm:
Tên: ${name}
Mô tả: ${description}
